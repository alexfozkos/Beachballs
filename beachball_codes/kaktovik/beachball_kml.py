...
def create_kml(csv_dict, args):
    ...
    fld = KML.Folder()
    for row in csv_dict:
        icon_path = "beachballs/%s.png" % (row["origin.evid"])
        km.Document.append(
            KML.Placemark(
                KML.Name(row["origin.evid"]),
                KML.Point(
...
def make_beachballs(csv_dict, args):
    ...
    for row in csv_dict:
        ...
        beachball(mt, facecolor=args.c, outfile="%s/beachballs/%s.png" % (args.output_path,row["origin.evid"]))
